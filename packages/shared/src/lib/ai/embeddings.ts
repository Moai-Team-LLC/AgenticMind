...
const createLocalProvider = (modelId: string, pooling: "cls" | "mean"): EmbeddingsProvider => {
  ...
  const getPipe = (): Promise<FeatureExtractionPipeline> => {
    pipe ??= pipeline("feature-extraction", modelId) as Promise<FeatureExtractionPipeline>
    return pipe
  }
  return {
...
const createOpenAiProvider = (
  ...
  return {
    ...
    embed: async (text, purpose = "embed") => {
      const rows = await pRetry(() => callOnce(text), buildRetryOptions(purpose))
      const first = rows[0]
      if (first === undefined) {
    ...
    embedBatch: async (texts, purpose = "embed batch") => {
      ...
      return pRetry(() => callOnce(texts), buildRetryOptions(purpose))
    },
  }
}
