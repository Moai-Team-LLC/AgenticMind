...
const ADVISORY_LOCK_KEY = 4242042
...
export const startKnowledgeFeedbackScheduler = (): { stop: () => void } => {
  ...
  const schedule = (): void => {
    ...
    console.log(
      `[WORKER] ${new Date().toISOString()}: next knowledge-feedback sweep in ~${Math.round(delay / 60000)} min`,
    )
    timer = setTimeout(() => {
      void runGuarded()
        .catch((e: unknown) => console.error("[WORKER] knowledge-feedback sweep error:", e))
        .finally(() => schedule())
    }, delay)
  }
  schedule()
