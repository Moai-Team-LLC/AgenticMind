...
import {
  ...
  type AskForEval,
  type EvalCase,
} from "@agenticmind/shared/lib/eval/harness"
...
const askForEval: AskForEval = async (query) => {
  ...
  if (!guard.ok) return { blocked: true, answer: "", citations: [] }
  const res = await ask({ tx: db, question: query, cardsEnabled, cacheEnabled })
  if (res.isErr()) return { blocked: false, answer: "", citations: [] }
  return {
    blocked: false,
    answer: res.value.answer,
    citations: res.value.citations.map((c) => ({ title: c.title, materialId: c.materialId })),
  }
}
