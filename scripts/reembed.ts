...
const newCounts = (): Counts => ({ total: 0, updated: 0, skipped: 0, failed: 0 })
    ...
    if (embeddable.length > 0) {
      ...
      if (embedded.isErr()) {
        ...
        for (let i = 0; i < embeddable.length; i++) {
          ...
          try {
            await props.write(row.id, vector)
            counts.updated += 1
          } catch (error) {
            counts.failed += 1
            console.warn(
              `[REEMBED] ${props.name} row ${row.id} write failed: ${
...
const reembedChunks = (tx: Transaction): Promise<Counts> =>
  reembedTable({
    tx,
    name: "chunks",
    load: (limit, offset) =>
...
const reembedCards = (tx: Transaction): Promise<Counts> =>
  reembedTable({
    tx,
    name: "cards",
    load: (limit, offset) =>
...
const reembedBeliefs = (tx: Transaction): Promise<Counts> =>
  reembedTable({
    tx,
    name: "beliefs",
    ...
    load: async (limit, offset) => {
      const rows = await tx
        ...
        .limit(limit)
        .offset(offset)
      return rows.map((r) => ({ id: r.id, text: `${r.subject} ${r.predicate} ${r.object}` }))
    },
