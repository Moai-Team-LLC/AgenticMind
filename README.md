...
**An auditable, self-improving knowledge & memory layer for agents.**

Not "memory storage for an agent." AgenticMind is the substrate an agent points
at when it needs (1) grounded answers with **provable citations**, (2) a full
**why-trace** for every answer, and (3) a corpus that **improves itself** from
...
> A reference implementation of **[the Agentic Product Standard](https://github.com/AlexDuchDev/agentic-product-standard)** — it puts the standard's memory-architecture, MCP tool-design, eval calibration, and observability layers into running code.

## What it does

A tiered retrieval + memory engine that an agent consumes over **MCP**:

- **Tier 0 — chunks**: hybrid vector + full-text retrieval (pgvector).
- **Tier 1 — cards**: typed, ontology-validated facts distilled from materials.
- **Tier 2 — graph**: entity/relation extraction + multi-hop traversal (graph module).
- **Synthesis**: citation-enforced answers — every claim keyed to a source.
- **Compounding loop**: validated answers get promoted into the corpus, so it
  gets better over time.
- **Trace**: every decision is logged and replayable for audit.
...
There is **no frontend** — the only consumers are agents over MCP. The tool
logic is framework-agnostic in `packages/shared/src/lib/knowledge/mcp-tools.ts`;
the host is ~60 lines of `Bun.serve` (no Next, no React).
...
`http://localhost:3000/mcp` with a bearer `typ="mcp"` JWT. (Lint additionally
...
Agent-first and **Postgres-only**: the graph lives behind a `GraphStore` interface
(recursive-CTE traversal on Postgres, no extra service), compounding is driven by
**programmatic** (not human) signals, MCP tokens are scoped least-privilege, the agent
principal is slim, and the host is headless Bun. The knowledge engine is English-language
(`simple` + `english` full-text configs alongside pgvector embeddings).
